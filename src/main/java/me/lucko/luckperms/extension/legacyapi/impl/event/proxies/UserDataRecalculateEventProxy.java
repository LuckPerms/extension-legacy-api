...
public class UserDataRecalculateEventProxy implements UserDataRecalculateEvent {
    private final LegacyApiProvider api;
    private final net.luckperms.api.event.user.UserDataRecalculateEvent event;

    public UserDataRecalculateEventProxy(LegacyApiProvider api, net.luckperms.api.event.user.UserDataRecalculateEvent event) {
        this.api = api;
        this.event = event;
    }
    ...
    public @NonNull Class<? extends LuckPermsEvent> getEventType() {
        return UserDataRecalculateEvent.class;
    }
}
